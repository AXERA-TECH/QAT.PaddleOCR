...
from pytorchocr.training import build_rec_model, load_ocr_config, rec_out_channels


def parse_args():
    parser = argparse.ArgumentParser(
        description="Compare PP-OCR recognition CTC logits in Paddle and PyTorch."
    )
...
def build_paddle(config, config_path, weights_path):
    ...
    model.eval()
    # Keep all BN layers in inference mode while exposing pre-softmax CTC logits.
...
def build_torch(config_path, weights_path):
    ...
    wrapper.model.eval()
    wrapper.model.head.ctc_head.train()
    return wrapper
...
def main(args):
    ...
    paddle_model = build_paddle(config, args.model_config, args.paddle_weights)
    torch_model = build_torch(args.model_config, args.torch_weights)
    with paddle.no_grad():
        paddle_output = paddle_model(paddle.to_tensor(images)).numpy()
    with torch.no_grad():
    ...
    paddle_probabilities = torch.softmax(
        torch.from_numpy(paddle_output),
        dim=-1,
    ).numpy()
    ...
    probability_difference = np.abs(paddle_probabilities - torch_probabilities)
    ...
    result = {
        ...
        "argmax_agreement": float(
            np.mean(
                np.argmax(paddle_output, axis=-1)
                == np.argmax(torch_output, axis=-1)
            )
        ),
