...
class QuantConf:
    ...
    output_is_symmetric: Optional[bool] = None
...
def get_quantization_config(
    ...
    if is_qat:
        ...
        elif input_dtype is not None and input_dtype.dtype in (
            torch.int16,
            torch.uint16,
        ):
            act_observer_or_fake_quant_ctr = FakeQuantize
    ...
    if output_dtype is not None:
        output_qscheme = (
            ...
            if quant_config.output_is_symmetric
            else torch.per_tensor_affine
            if quant_config.output_is_symmetric is not None
            else act_qscheme
        )
    ...
    if is_qat:
        ...
        weight_observer_or_fake_quant_ctr = (
            ...
            and weight_dtype.dtype in (torch.int16, torch.uint16)
...
def load_global_config(global_config: Dict[str, str], is_qat: bool = True):
    is_symmetric, quant_config = get_config(
        global_config,
        inherit_output_from_input=True,
    )
...
def load_regional_config(regional_config: Dict[str, str], is_qat: bool = True):
    ...
    is_symmetric, quant_config = get_config(
        regional_config["module_config"],
        inherit_output_from_input=False,
    )
