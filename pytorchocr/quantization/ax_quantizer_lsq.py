# Copyright (c) 2026 AXERA-TECH.
# Vendored from QAT.Ultralytics.YOLOv5 (AGPL-3.0); see UPSTREAM.md.
# Local changes: package-relative imports only.
...
def get_quantization_config(
    ...
    extra_args: Dict[str, Any] = {"eps": 2**-12, "use_grad_scaling": True}
    ...
    if output_dtype is not None:
        output_symmetric = quant_config.output_is_symmetric
        if output_symmetric is None:
            output_symmetric = is_symmetric
        output_qscheme = (
            torch.per_tensor_symmetric if output_symmetric
    ...
    if is_qat:
        ...
        weight_observer_or_fake_quant_ctr = _LearnableFakeQuantize
    else:
        extra_args["observer"] = PerChannelMinMaxObserver
...
def get_config(config: Dict[str, Any]):
    ...
    if output_is_symmetric is not None:
        output_is_symmetric = bool(output_is_symmetric)
    if config["input"]["dtype"] == "FP32":
        ...
        quant_config = QuantConf(
            ...
            output_is_symmetric=output_is_symmetric,
        )
...
class AXQuantizer(Quantizer):
    ...
    def __init__(
        ...
        if config_file is not None:
            global_config, regional_configs = load_config(
