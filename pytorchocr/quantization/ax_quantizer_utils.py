...
def _set_output_qspec(output_node: Node, output_qspec: QuantizationSpecBase) -> None:
    annotation = output_node.meta.get("quantization_annotation")
    if annotation is None:
        annotation = QuantizationAnnotation()
        output_node.meta["quantization_annotation"] = annotation
    annotation.output_qspec = output_qspec
    ...
    for node in gm.graph.nodes:
        ...
        if is_global:
            ...
            if output_qspec is not None:
                _set_output_qspec(output_node, output_qspec)
...
def _annotate_conv(
    ...
    def conv2d_same(x, conv_weight, conv_bias):
        return F.conv2d(x, conv_weight, conv_bias, padding="same")
    ...
    combinations = [
        ...
        (conv2d_same, True, _conv2d_bn_example_inputs),
        (conv2d_same, False, _conv2d_example_inputs),
    ]
    activations = [
        "relu",
        "relu_inplace",
    ...
    for node in gm.graph.nodes:
        ...
        if is_global:
            ...
            if output_act_qspec is not None:
                _set_output_qspec(output_node, output_act_qspec)
...
def _is_scalar(gm, value, expected):
    if isinstance(value, Node) and value.op == "get_attr":
        value = getattr(gm, value.target)
        if isinstance(value, Tensor) and value.numel() == 1:
            value = value.item()
    return isinstance(value, (int, float)) and abs(float(value) - expected) < 1.0e-6


@register_annotator("hardsigmoid")
def _annotate_hardsigmoid(
    gm: torch.fx.GraphModule,
    quantization_config: Optional[QuantizationConfig],
    module_names: List[str] = None,
    is_global: bool = True,
) -> Optional[List[List[Node]]]:
    # Paddle's hard sigmoid uses relu6(1.2 * x + 3) / 6. The slope Mul remains
    # an independently quantized op; the hard-sigmoid partition starts at Add.
    relu6_ops = (
    ...
    for output_node in gm.graph.nodes:
        if (
            output_node.op != "call_function"
            or output_node.target != torch.ops.aten.div.Tensor
            or not _is_scalar(gm, output_node.args[1], 6.0)
        ):
            continue
        relu6_node = output_node.args[0]
        if not isinstance(relu6_node, Node) or relu6_node.target not in relu6_ops:
            continue
        add_node = relu6_node.args[0]
        if (
            not isinstance(add_node, Node)
            or add_node.target != torch.ops.aten.add.Tensor
            or not _is_scalar(gm, add_node.args[1], 3.0)
        ):
            continue
        scale_node = add_node.args[0]
        if (
            not isinstance(scale_node, Node)
            or scale_node.target != torch.ops.aten.mul.Tensor
            or not _is_scalar(gm, scale_node.args[1], 1.2)
        ):
            continue
        partition = [add_node, relu6_node, output_node]
        if is_global:
            if _is_annotated(partition):
                continue
            add_node.meta["quantization_annotation"] = QuantizationAnnotation(
                input_qspec_map={
                    scale_node: get_input_act_qspec(quantization_config),
                },
                _annotated=True,
            )
            output_node.meta["quantization_annotation"] = QuantizationAnnotation(
            ...
            if not _is_annotated(partition):
                raise AssertionError("Paddle hard sigmoid was not globally annotated.")
            if module_names is not None and output_node.name not in module_names:
                continue
            add_node.meta["quantization_annotation"].input_qspec_map = {
                scale_node: get_input_act_qspec(quantization_config),
            }
            output_node.meta["quantization_annotation"].output_qspec = (
                get_output_act_qspec(quantization_config)
            )
