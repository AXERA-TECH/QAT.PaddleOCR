...
import numpy as np
from pytorchocr.modeling.backbones.rec_svtrnet import Mlp
...
class Transformer(nn.Module):
    ...
    def forward_train(self, src, tgt):
        ...
        tgt_mask = self.generate_square_subsequent_mask(tgt.shape[1], tgt)
    ...
    def forward_test(self, src):
        ...
        for len_dec_seq in range(1, self.max_len):
            ...
            tgt_mask = self.generate_square_subsequent_mask(
                dec_seq_embed.size(1), dec_seq_embed
            )
            tgt = dec_seq_embed
            for decoder_layer in self.decoder:
                tgt = decoder_layer(tgt, memory, self_mask=tgt_mask)
    ...
    def generate_square_subsequent_mask(self, sz, reference):
        """Generate a square mask for the sequence. The masked positions are filled with float('-inf').
            Unmasked positions are filled with float(0.0).
        """
        mask = reference.new_zeros([sz, sz], dtype=torch.float32)
        mask_inf = torch.triu(
            reference.new_full(
                (sz, sz),
